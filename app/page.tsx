...
export default function Home() {
  ...
  async function extractPDFText(file: File) {
    try {
      // dynamic import عشان مايشتغلش وقت البناء
      const pdfjsLib = await import("pdfjs-dist");

      // لازم تحدد worker src كده مع Turbopack
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-16">
          ...
          <Link href="/exam" className="bg-yellow-500 text-black px-6 py-3 rounded-2xl font-semibold">
            Exam Solver
          </Link>
        </div>

        <section className="grid lg:grid-cols-2 gap-10">
          <div className="bg-zinc-900 rounded-3xl p-6 border-zinc-800">
            <h2 className="text-3xl font-bold mb-6">Study Assistant</h2>
            ...
            <button onClick={generateStudyContent} className="bg-green-600 px-6 py-3 rounded-2xl">
          ...
          <div className="space-y-6">
            <div className="bg-zinc-900 rounded-3xl p-6">
              ...
              <div className="bg-black rounded-2xl p-5 min-h-[250px] overflow-auto">
                {loading? <p>Generating...</p> : <p className="whitespace-pre-wrap">{summary}</p>}
              </div>
            </div>

            <div className="bg-zinc-900 rounded-3xl p-6">
              <h3 className="text-3xl font-bold mb-6">Quiz Questions</h3>
